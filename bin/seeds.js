    ...
    return books.map(book => {
      return Author.findOne({ name: book.author.name, lastName: book.author.lastName })
        .then(author => {
          ...
          const bookWithReference = Object.assign({}, book, { author: [author._id] })
          console.log("Book with author reference:", bookWithReference.title, bookWithReference.author);
          return bookWithReference;
        })
    });
  })
  .catch(error => {
...
// I would prefer client side Id generation ....
const authorsArray = [
  {
